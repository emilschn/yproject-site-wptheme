...
//CACHE PROJECT CONTENT SUMMARY
        ...
        if ($cache_content_summary !== FALSE) {
            ...
            if ($campaign->video() == '' || $vote_status == 'preview') {
                ...
                foreach ($attachments as $attachment) {
                    if ($attachment->post_title == 'image_home')
                        $image_obj = wp_get_attachment_image_src($attachment->ID, "full");
                }
                //Sinon on prend la première image rattachée à l'article
                if ($image_obj == '' && count($attachments) > 0)
                    $image_obj = wp_get_attachment_image_src($attachments[0]->ID, "full");
                if ($image_obj != '')
                    $img_src = $image_obj[0];
...
//				$owner_str .= '@' . $author->user_nickname;
            ...
            <div id="project-map">
                ...
                <div id="map-cursor" style="<?php if ($cursor_top_position != '')
                echo 'top:' . $cursor_top_position . ';';
            if ($cursor_left_position != '')
                echo 'left:' . $cursor_left_position;
            ?> ">
...
<?php if (is_user_logged_in()) { ?>
    ...
    <div class="center">
        ...
        <div id="header_homepage_link" class="mobile_hidden">
            ...
            locate_template('page-connexion-lightbox.php', true);
